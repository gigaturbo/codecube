...
- [x] **BREAKING** the world is now a finite tray, **2048 nodes across** instead of roughly 8160. There is a bedrock wall at each edge, standing full height so you see it well before you reach it, and a bedrock floor you cannot dig, with nothing generated below it. The floor is eight nodes under the surface, so you meet it only where a program has cleared a shaft down to it. An existing world is re-bounded to the new size when you open it, so ground beyond 1024 that you have already visited is now outside the wall. The drone shares the same limit, so it can never build where you cannot walk
