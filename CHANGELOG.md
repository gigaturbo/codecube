...
- [x] **BREAKING** the world is now a finite tray, **2048 nodes across** instead of roughly 8160. There is a bedrock floor you cannot dig and a bedrock wall at each edge, both visible before you reach them, and nothing is generated below the floor. An existing world is re-bounded to the new size when you open it, so ground beyond 1024 that you have already visited is now outside the wall. The drone shares the same limit, so it can never build where you cannot walk
- [x] **You can no longer fall out of the world.** If you end up outside it anyway - a program can `remove` a floor tile, since bedrock is an ordinary node to the drone - you are put back at the spawn point, and the game makes sure the place it puts you is one you can stand in rather than mid-air or inside rock
- [x] Server owners can set the world's size from the settings menu, under Content: Games → Codecube
