...
- `menu/license.txt` — the three menu images, and any `.svg` source kept beside
  them. `menu/` is deliberately not export-ignored, because the main menu reads
  `menu/*.png` at runtime, so this file ships with them and is what a player
  finds. A vector source is the same work under the same licence and never ships,
  `*.svg export-ignore` catching it wherever it sits; `background.png` is a
  capture of the game itself rather than drawn art.
