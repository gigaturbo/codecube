...
- `menu/license.txt` — the three menu images, and the two `.svg` sources there
  are for them. `menu/` is deliberately not export-ignored, because the main menu
  reads `menu/*.png` at runtime, so this file ships with them and is what a
  player finds. The `.svg` sources are the same works under the same licence and
  do not ship (`*.svg export-ignore`); `background.png` has none, being a capture
  of the game itself.
