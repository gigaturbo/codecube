...
| A licence file states the *rule* an `export-ignore` glob already enforces, and never enumerates the sources that happen to exist — `*.svg` is a glob, the menu artwork's sources changed twice in one day, and both times prose in a file that ships was left naming a deleted image | — | `menu/license.txt`, `THIRD-PARTY-LICENSES.md` |
